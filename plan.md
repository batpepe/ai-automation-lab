...
- [x] MCP server driven end to end over stdio against the live cluster: all six tools advertised, `get_pod_status` returned five running pods from `apps` in 133ms, `get_recent_deploys` returned real ArgoCD sync history. Driven through the MCP protocol itself, which is what an editor does.
- [x] MCP and in-process parity test: same names, same schemas, same descriptions, same results.

**Measured during the phase:** running the tools against the real cluster found an over-redaction bug the fixtures never would have: a bare `key` field is structural in Kubernetes (taint keys, ConfigMap item names), and treating it as a secret hid why a pod would not schedule. Separately, `asyncio.timeout` cancels an awaiting task but cannot cancel the worker thread beneath it, so the synchronous Kubernetes client kept a socket open well past its budget when the cluster became unreachable. The blocking call now carries its own `_request_timeout` and a test pins it. Also measured: the n8n-style trap of trusting a library's own escaping - a pydantic model passed to MCP as one parameter nests every argument under `params`, which would have made the MCP surface and the agent surface two different tools.
